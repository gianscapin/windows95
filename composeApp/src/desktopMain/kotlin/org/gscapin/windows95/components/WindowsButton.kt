...
@Composable
fun WindowsButton(modifier: Modifier, dotPadding: Dp = 0.dp, onClick: () -> Unit, content: @Composable () -> Unit) {
    ...
    Box(modifier.background(backgroundComponent).drawBehind {
        ...
        Box(modifier = Modifier.offset {
            ...
            if(isPressed && dotPadding != 0.dp) {
                drawRect(
                    color = Color.Black,
                    style = Stroke(width = 2f, pathEffect = PathEffect.dashPathEffect(floatArrayOf(2f, 2f), phase = 0f))
                )
            }
        }) {
            content()
