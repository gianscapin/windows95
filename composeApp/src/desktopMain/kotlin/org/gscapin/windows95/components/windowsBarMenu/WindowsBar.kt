...
import org.gscapin.windows95.components.WindowsButton
...
fun WindowsBar(onWindowsButtonSelected: () -> Unit) {
    ...
    Column {
        ...
        Row(
            modifier = Modifier.height(40.dp).fillMaxWidth().background(backgroundComponent), verticalAlignment = Alignment.CenterVertically
        ) {
//            Box(Modifier.padding(start = 8.dp).fillMaxHeight().width(80.dp).background(windowsBlue))
            WindowsButton(Modifier.padding(start = 8.dp).height(36.dp).width(95.dp), onClick = {
                onWindowsButtonSelected()
            }){
